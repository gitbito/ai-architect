...
- **Google Gemini** — Gemini API key (required); supported models: `gemini-2.5-flash-lite`, `gemini-3-flash-preview` and `gemini-3.1-pro-preview` (leave blank to use the default model)
...
- **Local OpenAI-compatible** — Connect AI Architect to your own OpenAI-compatible `/v1` endpoint, such as vLLM, Ollama, LM Studio, LocalAI, TGI, or llama.cpp, so inference stays within your network. Provide the base URL including `/v1` (for example, `https://llm.internal:8000/v1`) and the exact model name served by your endpoint — recommended model: **`claude-haiku-4.5`**. API token is optional; if required, it is sent as a Bearer token, and can be left blank for keyless local servers. The endpoint must be reachable from the AI Architect containers: for a server running on the same host, use `http://ai-architect-local:<port>/v1` instead of `localhost`, `127.0.0.1`, or `0.0.0.0`; for remote servers, use a private-network or VPN-resolvable hostname.
